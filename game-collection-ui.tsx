...
// API Configuration - UPDATE THESE WITH YOUR FASTAPI ENDPOINTS
const API_BASE_URL = 'http://localhost:8000'; // Change to your FastAPI URL

const API_ENDPOINTS = {
  getGames: `${API_BASE_URL}/games`,
  updateGame: (id) => `${API_BASE_URL}/games/${id}`,
  deleteGame: (id) => `${API_BASE_URL}/games/${id}`,
...
export default function GameCollectionUI() {
  ...
  const updateGameState = async (gameId, newState) => {
    try {
      // UNCOMMENT THIS WHEN YOUR API IS READY:
      // const response = await fetch(API_ENDPOINTS.updateGame(gameId), {
      //   method: 'PUT',
